...
and the v1 DDL that becomes Hasura migration 1.
...
D8  IDs are uuid (gen_random_uuid()) — Hasura-friendly, safe to expose
    to a client, no sequence to fight in a future replica/merge story.
...
transactions   the register (D1, D2, D3, D7). amount_cents signed:
               negative = money out. category_id NULL iff it is a
               transfer (transfer_id NOT NULL) — enforced by CHECK.
...
create table transactions (
  ...
  created_at    timestamptz not null default now(),
  updated_at    timestamptz not null default now(),
  -- a transfer leg has no category; a normal row has one
  check ((transfer_id is null) = (category_id is not null))
);
create trigger transactions_updated_at before update on transactions
  for each row execute function set_updated_at();

-- D7: the two indexes that make the read shapes real
...
-- D1: both legs of a transfer must exist and sum to zero. Deferred so
-- the two inserts can happen in one transaction in either order.
create or replace function transfers_balance() returns trigger
language plpgsql as $$
declare total bigint; legs int;
begin
  if new.transfer_id is null then return new; end if;
  select coalesce(sum(amount_cents),0), count(*) into total, legs
    from transactions where transfer_id = new.transfer_id;
  if legs <> 2 or total <> 0 then
    raise exception 'transfer % must have exactly 2 legs summing to 0 (has %, sum %)',
...
Track all five tables. Relationships Hasura infers from the FKs:
...
3. Apply migration 1 against `ledger` via a port-forward to the -rw
   service; verify with \dt and a transfer that violates the trigger.
